...
#else
// If Mac or Linux store in /tmp dir
char *xfer_tempdlname = "/tmp/download.tmp";
#endif
//char *xfer_tempdlname = "/tmp/download.tmp";
...
int xfer_recv(void) {
  ...
  if ((xfer_recvfile = fopen(xfer_tempdlname, "wb"))) {
    ...
    asprintf(&_debugMsg,"Attempt to open tempfile (%s) failed in xfer_recv", xfer_tempdlname);
    cfg_debug(_debugMsg);
    return(0);
  }
}
...
void xfer_send(char *filename) {
  ...
  if ((p = strrchr(cfg_xferdir, '.')) && strlen(p) == 4 && (p[1] == 'd' || p[1] == 'D') && isdigit(p[2]) && isdigit(p[3])) {
    if (xfer_copy_from_image(cfg_xferdir, filename, xfer_tempulname)) {
      ...
      menu_show();
      printf("Attempt to open file (%s) failed in xfer_send()",filename);
      gfx_vbl();
      return;
    }
  } else {
...
void xfer_save_file_in_image(char *filename) {
  ...
  if ((di = di_load_image(cfg_xferdir)) == NULL) {
    ...
    menu_show();
    printf("Attempt to open disk image (%s) in xfer_save_file_in_image() failed.",di->filename);
    gfx_vbl();
    return;
  }
  ...
  if ((from = fopen(xfer_tempdlname, "rb")) == NULL) {
    ...
    menu_show();
    printf("Attempt to open temp file (%s) in xfer_save_file_in_image() failed.",xfer_tempdlname);
    gfx_vbl();
    return;
  }
  ...
  if (to == NULL) {
    ...
    menu_show();
    printf("Attempt to write file (%s) in xfer_save_file_in_image() failed.",filename);
    gfx_vbl();
    return;
  }
...
void xfer_save_file_in_dir(char *filename) {
  ...
  if ((from = fopen(xfer_tempdlname, "rb")) == NULL) {
    menu_draw_message("Couldn't open temp file!");
    menu_show();
    printf("Attempt to open temp file (%s) in xfer_save_file_in_dir() failed!",xfer_tempdlname);
    gfx_vbl();
    return;
  }
  ...
  if ((to = fopen(name, "wb")) == NULL) {
    ...
    menu_draw_message("Couldn't write file!");
    menu_show();
    printf("Attempt to write file (%s) in xfer_save_file_in_dir() failed!",name);
    gfx_vbl();
    return;
  }
